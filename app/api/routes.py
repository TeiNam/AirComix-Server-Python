...
# 썸네일 서비스는 하나만 만들어 핸들러와 공유한다 (캐시 디렉토리/맵핑 상태 중복 방지)
...
async def get_server_info():
    ...
    """
    try:
        # 값은 설정에서 읽는다. 줄 구분자와 순서는 AirComix 앱 호환을 위해 유지한다.
        server_info = (
            f"allowDownload={settings.allow_download}\n"
            f"allowImageProcess={settings.allow_image_process}\n"
            f"{settings.server_name} - FastAPI 기반 만화 스트리밍 서버"
        )
...
async def health_check():
    ...
    인증 없이 접근 가능하므로 서버 내부 정보(경로, 설정)는 노출하지 않습니다.
    ...
    try:
        # 만화 디렉토리 접근 가능 여부 확인
        if not Path(settings.manga_directory).exists():
...
async def get_thumbnail(path: str):
    ...
    try:
        ...
        # resolve_safe_path 가 거부한 경로
        logger.warning(f"안전하지 않은 썸네일 경로: {path}, {e}")
...
@admin_router.get("/thumbnail/info")
...
@admin_router.post("/thumbnail/cleanup")
...
@admin_router.delete("/thumbnail/cache")
...
async def get_thumbnail_by_thm_extension(path: str):
    ...
    ponytail: 경로 파라미터를 `{path:path}` 로 넓히면 `/comix/...thm` 요청까지
    이 라우트가 가로채 `/comix/{path:path}` 핸들러의 중첩 썸네일 처리를 깨뜨린다.
    따라서 최상위 경로 전용으로 남겨두고, 중첩 경로는 handlers 쪽에서 처리한다.
    ...
    try:
        ...
        logger.warning(f"안전하지 않은 썸네일 경로 (.thm): {path}, {e}")
        raise HTTPException(status_code=403, detail="접근이 거부되었습니다")
    except Exception as e:
